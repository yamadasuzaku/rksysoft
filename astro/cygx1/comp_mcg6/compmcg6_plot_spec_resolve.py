...
def plot_bothdata(ene1, ene_err1, data1, data_err1, ene2, ene_err2, data2, data_err2, output_prefix, \
                  xlim=None, ylim=None, debug=False, color2="skyblue"):
    """
    Plots the data using matplotlib and saves the plot as .png and .eps files.
    ...
    # インセットプロット領域
    ax2 = fig.add_axes([0.58, 0.58, 0.39, 0.32])
    ...
    corfactor = 0.1
    errorbar = ax1.errorbar(ene2, data2/corfactor, xerr=ene_err2, yerr=data_err2/corfactor, fmt='.', color=color2, ms=1, label=f"MCG-6-30-15 (1/{corfactor})")
    for line in errorbar[2]: # errorbar[2] は誤差棒の Line2D オブジェクト
        line.set_alpha(0.3)  # 誤差棒の透明度を0.3に設
...
#    ax1.grid(True,alpha=0.2)
    ...
    errorbar = ax2.errorbar(ene2, data2/corfactor, xerr=ene_err2, yerr=data_err2/corfactor, fmt='.', color=color2, ms=1, label=f"MCG-6-30-15 (1/{corfactor})")
    for line in errorbar[2]: # errorbar[2] は誤差棒の Line2D オブジェクト
        line.set_alpha(0.3)  # 誤差棒の透明度を0.3に設
    errorbar = ax2.errorbar(ene2, data2/corfactor, xerr=ene_err2, yerr=data_err2/corfactor, fmt='-', color=color2, alpha=0.1)
...
#    errorbar = ax2.errorbar(ene2, data2/corfactor, xerr=ene_err2, yerr=data_err2/corfactor, fmt='.', color='skyblue', ms=1, label=f"Xtend (1/{corfactor})")
    # for line in errorbar[2]: # errorbar[2] は誤差棒の Line2D オブジェクト
    #     line.set_alpha(0.3)  # 誤差棒の透明度を0.3に設
    ax2.set_xlim(6.2,7.2)
    ax2.set_ylim(0,3.1)
    ax2.grid(True,alpha=0.75, axis='x', linestyle="--")
    # 不要な枠線を削除します
    ...
    ax2.text(6.395, 3.15, r"Fe I", rotation=70, color="salmon", fontsize=10)
...
def main():
    ...
    # Plot both data
    plot_bothdata(ene1, ene_err1, data1, data_err1, ene2, ene_err2, data2, data_err2, args.output, \
                    xlim=args.xlim, ylim=args.ylim, debug=args.debug, color2=color2)
