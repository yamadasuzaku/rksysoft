...
def plot_arc_with_tics_on_axis(axis, center, radius, angles_degrees, tics_length=0.05, label_offset=0.2, offset_angle = 90.0, paper=True):
    """
    ...
    for i, angle in enumerate(angles_degrees):
        ...
        if i%2==0:
            if paper:
                # revserve sign of angle just for visual clarity
                axis.text(label_x, label_y, f"{-1 * angle}$^\circ$", ha='center', va='center', fontsize=10, color='navy')
            else:
...
def plot_binary_system_with_velocity(
    M_bh, M_star, R_star, distance_cygx1, grid_size=20, grid_range_factor=1.6, 
    ...
    paper_symetric_factor = 0.2
    dsym = - grid_range * paper_symetric_factor
    x = np.linspace(-grid_range + dsym, grid_range + dsym, grid_size)
    ...
    ############# plot ######################################
    nrows, ncols = 2, 1 
#    fig, axes = plt.subplots(figsize=(5, 8), nrows=nrows, ncols=ncols, tight_layout=True)

    fig, axes = plt.subplots(figsize=(5, 9), nrows=nrows, ncols=ncols, 
                         gridspec_kw={'height_ratios': [1.7, 1]},  # 上の図が20%大きい
                         tight_layout=True)

    angles_num_points=20    
    angles = np.linspace(+np.pi/6, +5*np.pi/6, angles_num_points, endpoint=True)
    ...
    # 描画設定
    axes[0].set_xlim(-grid_range + dsym, grid_range + dsym)
    axes[0].set_ylim(-grid_range, grid_range)
    axes[0].set_aspect('equal')
    if not paper:
        axes[0].set_title(title)
    axes[0].set_xlabel(f"X"+r"($R_{\odot}$)")
    axes[0].set_ylabel(f"Y"+r"($R_{\odot}$)")
#    axes[0].set_xlabel(f"X [{unit_label}]")
#    axes[0].set_ylabel(f"Y [{unit_label}]")
    axes[0].grid(alpha=0.1)

    # plot BH and star
    axes[0].plot(bh_x, 0, 'ko', markersize=2, label='BH')  # ブラックホール
    axes[0].plot(star_x, 0, 'bs', markersize=2, label='HD 226868')  # 伴星

    # 結果のプロット
    quiver = axes[0].quiver(
        X, Y, plot_Vx, plot_Vy, plot_Vmag,
        cmap=cmap, scale=arrow_scale, scale_units='xy', angles='xy', alpha=0.5
    )
    # 矢印のスケール説明を追加
    quiver_scale_length = 1000  # 矢印のスケール基準 (速度の大きさが quiver_scale_length のときの矢印の長さ)
    quiver_key = axes[0].quiverkey(
        quiver, X=0.73, Y=-0.1, U=quiver_scale_length, label=f"{quiver_scale_length} [km/s]",
        labelpos='E', coordinates='axes'
    )
    # Colorbar
    cbar = plt.colorbar(quiver, ax=axes[0], orientation="horizontal", pad=0.1, aspect=50, shrink=0.6)
    ...
    if plot_roche:
        ...
        x = np.linspace(-grid_range + dsym, grid_range  + dsym, grid_size*10)
        ...
        axes[0].contour(X, Y, Z_posi, levels=levels_line, colors="gray", linewidths=0.5, alpha=0.2)
    ...
    for idx, proj_fraction in enumerate([0.2,0.4,0.6,0.8]):
        ...
        axes[1].set_xlim(-61,61)
        axes[1].set_ylim(-1200,1200)

        if paper:
            # reverse sign of velocity just for visual 
            axes[1].plot(inclinations, proj_Vmag[::-1], '-', color=color, markersize=6, label=f"{proj_fraction}" + r"$\times d$")

            # x の範囲
            x_min, x_max = -27.5, 27.5 # optical inclination 

            # y の範囲 (1sigma)
            y_min, y_max = -141, -111
            # プロット用の x 値
            x = np.linspace(x_min, x_max, 100)
            axes[1].fill_between(x, y_min, y_max, color='pink', alpha=0.2)  # 半透明の薄い黄色で塗りつぶし

            # y の範囲 (3sigma)
            y_min, y_max = -171, -81
            # プロット用の x 値
            x = np.linspace(x_min, x_max, 100)            
            axes[1].plot([x_min, x_max, x_max, x_min, x_min], [y_min, y_min, y_max, y_max, y_min], color='orange', linestyle='-',alpha=0.5, lw=1)  # オレンジの点線枠
#            axes[1].fill_between(x, y_min, y_max, color='gray', alpha=0.5)  # 半透明の塗りつぶし

            # y = 0 の直線を点線で描く  
            axes[1].axhline(y=0, color='black', linestyle='dotted', linewidth=1, alpha=0.6)


        else:
            axes[1].plot(inclinations, proj_Vmag, 'o-', color=color, markersize=6, label=f"{proj_fraction}" + r"$\times d$")
    ...
    plt.savefig(f"{filename.replace(".png",".pdf")}")
    print(f"Plot saved as {filename.replace(".png",".pdf")}")
    plt.savefig(f"{filename.replace(".png",".eps")}")
    print(f"Plot saved as {filename.replace(".png",".eps")}")
