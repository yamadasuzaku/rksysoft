...
def parse_args():
    ...
    parser.add_argument('--paper', '-pa', action='store_true', help='Cyg X-1 first paper flag')
...
def setup_plot():
    ...
    params = {'xtick.labelsize': 12, 'ytick.labelsize': 12, 'legend.fontsize': 12, \
              'axes.labelsize': 12  # xlabel, ylabel のフォントサイズを変更}
              }
...
def plot_rate_vs_grade(event_list, plotpixels, itypenames, timebinsize, output, ref_time, \
                            gtiuse = False, debug=False, show = True, nonstop = False, \
                            lcthresh = 0.8, rate_max_ingratio=10, yscale_ingratio = "log", paper=False):
    """
    ...
    for fname in event_list:
        ...
        for j, pix in enumerate(plotpixels):
            ...
            ax.set_xlabel(r"Count rates of all grades (c s$^{-1}$ pixel$^{-1}$)")
            ax.set_ylabel(r"Count rate of each grade  (c s$^{-1}$ pixel$^{-1}$)")
            ax.grid(alpha=0.2,which="both",linestyle='--', linewidth=0.5)
            if paper:
                plt.figtext(0.15,0.90,f"Pixel={pix:02d} \nTime Binsize = {int(timebinsize)} (s)", fontsize=12)
            else:                
            ...
            if yscale_ingratio == "log":
                ...
                if paper:
                    ax.set_ylim(1e-4,rate_max_ingratio)
                    ax.set_xlim(1e-1,rate_max_ingratio)
            for k, itype_ in enumerate(itypenames):
                itypestr = g_typename[itype_]
                print(f"ピクセル{pix}とタイプ{itype_}のデータを処理中 (obsid {obsid})")
                type_color = type_colors[k % len(type_colors)]
                ...
                if paper:
                    ax.errorbar(pixel_y_lc, y_lc, fmt=ms_gratio, label=f"{itypestr}", color=type_color, ms=3)
                    ax.plot(rate_y,rate_y*bratios[itype_], "--", alpha=0.7,label=f"{itypestr}", color=type_color)
                else:
...
#            ax.legend(loc='upper left', bbox_to_anchor=(1, 1))
            plt.tight_layout()
            if paper:
                outpng = f"{output}_rate_vs_grade_pixel{pix:02d}_cygx1paper.png"
                plt.savefig(outpng)
                print(f"..... {outpng} is created. ")
                outeps = f"{output}_rate_vs_grade_pixel{pix:02d}_cygx1paper.eps"
                plt.savefig(outeps)
                print(f"..... {outeps} is created. ")
            else:                
    ...
    if args.gtiuse:
        ...
        if args.plot_rate_vs_grade:
            ...
            plot_rate_vs_grade(event_list, plotpixels, itypenames, args.timebinsize, args.output, ref_time,\
                gtiuse = args.gtiuse, debug= args.debug, show = args.show, nonstop = args.nonstop, lcthresh = args.lcthresh, \
                                            rate_max_ingratio = args.rate_max_ingratio, yscale_ingratio = args.yscale_ingratio, paper=args.paper)
