...
with h5py.File(args.input_file, "r") as f:
    ...
    # 4桁のchanと3桁以下のchanに分ける
    chan_keys_4digit = [key for key in chan_keys if re.match(r'^chan\d{4}$', key)]  # Bi TES
    chan_keys_short = [key for key in chan_keys if re.match(r'^chan\d{1,3}$', key)] # Sn TES

    print("4-digit chan keys:", chan_keys_4digit)
    print("3-digit or less chan keys:", chan_keys_short)
...
#    for chan in chan_keys:
    for chan in chan_keys_4digit:        
        timestamps = f[chan]['timestamp'][()]
        ene = f[chan]["energy"][()]

        all_timestamps.append(timestamps)
        all_energy.append(ene)
    
    # Convert the list of timestamps into a single numpy array
    all_timestamps_array = np.concatenate(all_timestamps)
    all_energy_arrray = np.concatenate(all_energy)

    # Get the sorting indices based on timestamps
    sorted_indices = np.argsort(all_timestamps_array)

    # Sort both timestamps and energy using the same indices
    all_timestamps_array = all_timestamps_array[sorted_indices]
    all_energy = all_energy_arrray[sorted_indices]
...
plt.text(0.05, 0.45, text_str, transform=plt.gca().transAxes, fontsize=10, verticalalignment='top', bbox=dict(facecolor='white', alpha=0.7))
...
plt.subplot(2, 2, 4)
plt.plot(remainders, all_energy, 'b.', markersize=1)
plt.ylim(0,100000)
plt.xlim(0.01,0.02)
