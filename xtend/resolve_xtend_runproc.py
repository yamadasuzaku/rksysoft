...
def main(args):
    ...
    for regfile in args.regfiles:
        ...
        subprocess.run(f"rm -rf {fitsfname}.img {fitsfname}.pha {fitsfname}.lc {fitsfname}.evt, {fitsfname}.arfregion0.reg", shell=True)
...
""")
    ...
    check_file_existance(file="xtd_source_R0.pha")
    subprocess.run(f'xtdrmf infile=xtd_source_R0.pha outfile={rmf_file} clobber=yes rmfparam=$CALDB/data/xrism/xtend/bcf/response/xa_xtd_rmfparam_20190101v004.fits', shell=True)
    check_file_existance(file=rmf_file)

    # xaexpmapによるexposure mapの作成
    print_status("===== Run xaexpmap =====", "info")
    expomap = "xtd_cl_evt.expo"
    subprocess.run(f'xaexpmap ehkfile={args.ehkfile} gtifile=xtd_source_R0.pha instrume=XTEND badimgfile={args.bad_pixel_image_file} pixgtifile={args.flickering_pixel_file} outfile={expomap} outmaptype=EXPOSURE delta=20.0 numphi=1 clobber=yes logfile=xtd_xaexpmap.log', shell=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--regfiles", nargs='+', default=["RegionFile/xtd_source_R0.reg"], help="List of region files to process")
    parser.add_argument("--eventfile", default="xa300049010xtd_p031300010_cl.evt", help="Event file")
    parser.add_argument("--ehkfile", default="xa300049010.ehk", help="EHK file")
    parser.add_argument("--bad_pixel_image_file", default="xa300049010xtd_p031300010.bimg", help="Bad pixel image file")
