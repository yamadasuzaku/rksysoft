        ...
        for itype in itypenames:
            ...
            if mask.any():
                ...
                if ratioflag:
                    ...
                    handle = ax[dety, detx].errorbar(energy, ratio, xerr=xerr, yerr=ratio_err, fmt='-',alpha=0.2, color="k")
                    handle = ax[dety, detx].errorbar(energy, ratio, xerr=xerr, yerr=ratio_err, fmt='.',alpha=0.9, color="k")
                    # Set error bar transparency
                    for bar in handle[2]:
                        bar.set_alpha(0.5)            

                else:
                    handle = ax[dety, detx].errorbar(energy, ncount, xerr=xerr, yerr=yerr, fmt='-',alpha=0.3, color="k")
                    handle = ax[dety, detx].errorbar(energy, ncount, xerr=xerr, yerr=yerr, fmt='.',alpha=0.9, color="k")
                    # Set error bar transparency
                    for bar in handle[2]:
                        bar.set_alpha(0.5)            
