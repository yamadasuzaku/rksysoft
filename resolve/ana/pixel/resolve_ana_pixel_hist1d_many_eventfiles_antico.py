...
def kev_to_pi(ev):
    ...
    return (ev - 0.1874) / 0.4274

def pi_to_kev(pi):
    """Convert PI units to energy in eV."""
    return pi * 0.4274 + 0.1874
...
def plot_xhist(file_names, x_col, x_hdu, outfname, xmin, xmax, rebin, \
                  plotflag=False, debug=True, filters=False, ylin=False, calout=True, itype_cut=0, PSP_ID=1, fixed_pi_lowlim=10.):

    if x_col == "PI":
        # convert Energy to PI (b/c user specify energy as arguments.) 
        pimin, pimax = kev_to_pi(xmin), kev_to_pi(xmax)
    else:
        pimin, pimax = xmin, xmax

    print(f"(pimin,pimax) = ({pimin},{pimax})  (xmin,xmax)=({xmin},{xmax})") 
    binnum = int((pimax - pimin) / rebin)
    print(f"(pimin,pimax) = ({pimin},{pimax})  (xmin,xmax)=({xmin},{xmax})  binnum = {binnum}") 
...
#    colors = plt.cm.viridis(np.linspace(0, 1, len(file_names)))    
    ...
    for i, file_name in enumerate(file_names):
        ...
        with fits.open(file_name) as hdul:
            ...
            itype = data["AC_ITYPE"] 
            len_pre = len(pspid)
            cutid = np.where((itype==itype_cut) & (pspid==PSP_ID))[0] # only PSPID=0 is used 
            data = data[cutid]
            len_post = len(pspid)
            ...
            if x_col == "PI":
                xval =  0.5 * (binedges[1:] + binedges[:-1]) * 0.4274 + 0.1874  # convert PI to energy (keV)
            else:
...
#            axs[0].errorbar(xval, hist, yerr=np.sqrt(hist), fmt='o', ms=2, alpha=0.9, color=colors[i], label=f"{short_filename} {target}" + f"({event_number:d} c)")
    ...
    axs[0].set_title(f"{outfname}_xmin{xmin}_xmax{xmax}_rebin{rebin}_itype{itype_cut}")
    ...
    axs[0].set_ylabel(f"Counts/bin (binsize={realbinsize:1.3f})")
...
#        axs[1].errorbar(xval, ratio_hist, fmt='o-', ms=2, alpha=0.9, label=f"Ratio {os.path.basename(file_name)}")
        axs[1].errorbar(xval, hist, yerr=np.sqrt(hist), fmt='o', ms=2, alpha=0.9, label=f"{file_name}")
    ...
    if x_col == "PI":
        ...
        axs[1].set_xlim(fixed_pi_lowlim,)
    else:
    ...
    ofname = f"{outfname}_xmin{xmin}_xmax{xmax}_rebin{rebin}_itype{itype_cut}.png"
    ...
    log_fname = f"{outfname}_xmin{xmin}_xmax{xmax}_rebin{rebin}_itype{itype_cut}_log.csv"
