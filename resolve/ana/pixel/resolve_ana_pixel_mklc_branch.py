...
# GTIを使用してデータを処理する関数
def process_data_wgti(fname, ref_time, timebinsize, debug=False, nonstop = False, output_dir = "output_dir"):
    """
    ...
    if debug:
        ...
        ax_debug.set_title(f"check overlap time zone \n {fname}", fontsize=8)
        ax_debug.grid(alpha=0.7)

        # 図の表示とクローズ
        plt.savefig(f"{output_dir}/fig_debug_datawgti_{outftag}.png")
#        plt.show()
        plt.close(fig_debug)
...
def plot_lightcurve(event_list, plotpixels, itypenames, timebinsize, output, ref_time, \
                       gtiuse = False, debug=False, show = False, nonstop = False, lcthresh = 0.8, output_dir="output_dir"):
    """
    ...
    for fname in event_list:
        ...
        if gtiuse:
            dt, time, dtime, pha, itype, rise_time, deriv_max, pixel, overlaps_start, overlaps_stop = process_data_wgti(fname, ref_time, timebinsize, debug=debug, nonstop = nonstop, output_dir = output_dir)
        else:
            dt, time, dtime, pha, itype, rise_time, deriv_max, pixel = process_data(fname, ref_time)
        ...
        for j, pix in enumerate(plotpixels):
            ...
            for k, itype_ in enumerate(itypenames):
                ...
                bratios = calc_branchingratios(pixel_y_lc, debug)
            ...
            plt.tight_layout()
            outpng = f"{output_dir}/{output}_lightcurve_pixel{pix:02d}_{outftag}.png"
            plt.savefig(outpng)
...
def plot_sumlightcurve(event_list, plotpixels, itypenames, timebinsize, output, ref_time, \
                       gtiuse = False, debug=False, show = False, nonstop = False, pfactor = 2, lcthresh = 0.8, output_dir="output_dir"):
    """
    イベントリストから光度曲線をプロットする。
    """
    colors = plt.cm.get_cmap('tab10', len(plotpixels)).colors
    ...
    for fname in event_list:
        ...
        for k, itype_ in enumerate(itypenames):
            ...
            plt.tight_layout()
            outpng = f"{output_dir}/{output}_sumlightcurve_type_{g_typename[itype_]}_{outftag}.png"
            plt.savefig(outpng)
            print(f"..... {outpng} is created. ")
            if show:
                plt.show()
...
#    ratio_ylim=(0, 10),           # for obs_over_model
    ...
    bratios = calc_branchingratios(rate_y, debug)  # expected branching ratio as function of total rate
    ...
    for fname in event_list:
        ...
        if gtiuse:
            dt, time, dtime, pha, itype, rise_time, deriv_max, pixel, overlaps_start, overlaps_stop = process_data_wgti(
                fname, ref_time, timebinsize, debug=debug, nonstop=nonstop, output_dir = output_dir
            )
        else:
            dt, time, dtime, pha, itype, rise_time, deriv_max, pixel = process_data(fname, ref_time)
        ...
        for j, pix in enumerate(plotpixels):
            ...
            outpng = f"{output_dir}/{output}_rate_vs_grade_pixel{pix:02d}_{outftag}.png"
    ...
    if save_summary_csv and len(summary_rows) > 0:
        ...
        outcsv = f"{output_dir}/{output}_rate_vs_grade_summary.csv"
...
def estimate_lp_ls_from_hpmpms_profilelik(
    *,
    ...
    fit_grades=(0, 1, 2),        # Hp/Mp/Ms を想定（itypeが数値コードの場合）
    target_grades=(3, 4),        # Lp/Ls を想定（itypeが数値コードの場合）
    ...
    """
    (A) Pixel総カウント版・最小実装（関数1本） + 診断図を追加

    方針:
      1) process_data(_wgti) で event-level を取得し、pixel×grade の総カウント K_{i,g} を作る
      2) 露光 T は
           - gtiuse=False: T = time[-1] - time[0]
           - gtiuse=True : T = sum(overlaps_stop - overlaps_start)
      3) fit_grades (=Hp/Mp/Ms想定) のみで λ_i を最尤推定:
           K_{i,g} ~ Poisson( T * λ_i * p_g(λ_i) )
         p_g(λ) は calc_branchingratios(rate_grid) を補間
      4) profile likelihood で λ_i の 68%CI（2ΔlogL<=1）
      5) λ_i,CI から Lp/Ls(=target_grades) の期待カウントを予測し、bias[%]とCIを出す
      6) CSV保存
      7) (NEW) bias[%] 図の前に、基礎量（Kobs/Mpred/T）診断図を生成
    ...
    def _build_branching_functions(rate_max_, ngrid_, itypenames_):
        ...
        br = calc_branchingratios(rate_grid, debug)

        def _get_br_array(g):
            # 1) dict / structured array: br[g]
            try:
                arr = br[g]
                return arr
            except Exception:
    ...
    # We will also collect per-file diagnostic tables:
    # diag[file_tag][pix][g] = {"Kobs":..., "Mpred":...} (Mpred available after λ fit)
    # T is per-file (not per-pixel) here.
    diag_tables = []
    ...
    for fname in event_list:
        ...
        # Optional debug prints (you can keep or remove)
        if debug:
        ...
        # Prepare diagnostic store for this file
        diag = {
            "obsid": obsid,
            "object": obj,
            "file": os.path.basename(fname),
            "outftag": outftag,
            "T_exposure_s": T,
            # per-pixel rows will be appended later
            "rows": [],
        }
        ...
        for pix in plotpixels:
            ...
            K_by_grade = {g: int(np.sum(cut_pix & (itype == g))) for g in itypenames}
            ...
            lam_hat, (lam_lo, lam_hi) = _mle_and_ci_profile(T, K_by_grade, p_of, lam_min, lam_max)

            # Predict expected counts for ALL grades (for diagnostics),
            # and also for target grades for CSV+bias plot.
            M_by_grade = {}
            Mlo_by_grade = {}
            Mhi_by_grade = {}
            ...
            for g in itypenames:
                if np.isfinite(lam_hat):
                    M_by_grade[g] = float(_M(g, lam_hat))
                else:
                    M_by_grade[g] = np.nan

                Mlo_by_grade[g] = float(_M(g, lam_lo)) if np.isfinite(lam_lo) else np.nan
                Mhi_by_grade[g] = float(_M(g, lam_hi)) if np.isfinite(lam_hi) else np.nan

            # bias for target grades
            bias = {}
            bias_lo = {}
            bias_hi = {}
            ...
            for g in target_grades:
                ...
                Mhat = M_by_grade.get(g, np.nan)
                Mlo = Mlo_by_grade.get(g, np.nan)
                Mhi = Mhi_by_grade.get(g, np.nan)

                if np.isfinite(Mhat) and Mhat > 0:
                    b = (Kobs - Mhat) / Mhat * 100.0
                else:
                    b = np.nan

                # conservative mapped interval
            ...
            for g in target_grades:
                ...
                row[f"Mpred_{g}"] = M_by_grade.get(g, np.nan)
                row[f"Mpred_lo_{g}"] = Mlo_by_grade.get(g, np.nan)
                row[f"Mpred_hi_{g}"] = Mhi_by_grade.get(g, np.nan)
                row[f"biaspct_{g}"] = bias.get(g, np.nan)
                row[f"biaspct_lo_{g}"] = bias_lo.get(g, np.nan)
                row[f"biaspct_hi_{g}"] = bias_hi.get(g, np.nan)

            results.append(row)

            # Row for diagnostics (store ALL grades)
            diag_row = {
                "pixel": pix,
                "lambda_hat_cps": lam_hat,
                "lambda_lo_cps": lam_lo,
                "lambda_hi_cps": lam_hi,
                "K_by_grade": K_by_grade,
                "M_by_grade": M_by_grade,
                "Mlo_by_grade": Mlo_by_grade,
                "Mhi_by_grade": Mhi_by_grade,
            }
            diag["rows"].append(diag_row)
    ...
    # Save CSV
    # -------------------------
    outcsv = f"{output_dir}/{output}_lp_ls_infer_profilelik.csv"
    ...
    # -------------------------
    # NEW: Diagnostic plots (basic quantities)
    # -------------------------
    outpng_diag = f"{output}_lp_ls_diag_basic.png"
    if make_plot and make_diagnostic_plot and len(results) > 0 and len(diag_tables) > 0:
        # For simplicity (and clarity), make one diagnostic page PER FILE.
        # If you pass multiple event_list entries, each makes its own png.
        for diag in diag_tables:
            rows = diag["rows"]
            if len(rows) == 0:
                continue

            # Sort by pixel
            rows_sorted = sorted(rows, key=lambda r: int(r["pixel"]))
            pixs = np.array([int(r["pixel"]) for r in rows_sorted], dtype=int)

            # Prepare K and M matrices: shape (n_pix, n_grade)
            grades = list(itypenames)
            Kmat = np.zeros((len(rows_sorted), len(grades)), dtype=float)
            Mmat = np.zeros_like(Kmat)
            for i, r in enumerate(rows_sorted):
                for j, g in enumerate(grades):
                    Kmat[i, j] = float(r["K_by_grade"].get(g, 0))
                    Mmat[i, j] = float(r["M_by_grade"].get(g, np.nan))

            # Figure layout:
            #  - Top: exposure time (single horizontal line / text) + lambda_hat vs pixel
            #  - Middle: Kobs vs pixel for each grade (multiple panels)
            #  - Bottom: Kobs and Mpred overlaid vs pixel for each grade (multiple panels)
            nG = len(grades)
            nrows = 2 + nG  # 2 summary rows + grade panels
            fig = plt.figure(figsize=(14, 1.5 * nrows))

            # Row 1: lambda_hat vs pixel
            ax0 = plt.subplot2grid((nrows, 1), (0, 0))
            lam_hat_arr = np.array([r["lambda_hat_cps"] for r in rows_sorted], dtype=float)
            ax0.plot(pixs, lam_hat_arr, marker="o", linestyle="-", alpha=0.9)
            ax0.set_ylabel("lambda_hat [c/s/pix]")
            ax0.grid(alpha=0.2)
            ax0.set_title(f"DIAGNOSTICS  OBSID={diag['obsid']}  OBJECT={diag['object']}  file={diag['file']}  T={diag['T_exposure_s']:.1f}s", fontsize=9)

            # Row 2: exposure time shown as text + optional constant line
            axT = plt.subplot2grid((nrows, 1), (1, 0), sharex=ax0)
            axT.plot(pixs, np.full_like(pixs, diag["T_exposure_s"], dtype=float), marker="o", linestyle="--", alpha=0.7)
            axT.set_ylabel("Texp [s]")
            axT.grid(alpha=0.2)

            # Rows 3..: per-grade Kobs and overlay Kobs vs Mpred
            # We’ll do for each grade: left axis as Kobs, overlay Mpred with dashed line.
            for j, g in enumerate(grades):
                ax = plt.subplot2grid((nrows, 1), (2 + j, 0), sharex=ax0)
                ax.plot(pixs, Kmat[:, j], marker="o", linestyle="-", alpha=0.9, label=f"Kobs (g={g})")
                ax.plot(pixs, Mmat[:, j], marker=".", linestyle="--", alpha=0.9, label=f"Mpred (g={g})")
                ax.set_ylabel(f"counts (g={g})")
                ax.grid(alpha=0.2)
            ...
            # Save per file (avoid overwriting if multiple)
            outpng_diag_i = f"{output_dir}/{output}_lp_ls_diag_basic_{diag['outftag']}.png"
            plt.savefig(outpng_diag_i, dpi=300)
            print(f"[OK] Diagnostic plot saved: {outpng_diag_i}")
    ...
    if make_plot and len(results) > 0:
        ...
        fig.suptitle("Lp/Ls bias[%] vs model (λ inferred from fit_grades) with profile-likelihood CI", y=0.995)
        fig.tight_layout()
        plt.savefig(f"{output_dir}/{outpng}", dpi=200)
    ...
    if args.gtiuse:
        if args.plot_lightcurve:
            ...
            plot_lightcurve(event_list, plotpixels, itypenames, args.timebinsize, args.output, ref_time,\
                gtiuse = args.gtiuse, debug= args.debug, show = args.show, nonstop = args.nonstop, lcthresh = args.lcthresh, output_dir=args.output_dir)
        ...
        if args.plot_rate_vs_grade:
            ...
            plot_rate_vs_grade(event_list, plotpixels, itypenames, args.timebinsize, args.output, ref_time,\
                gtiuse = args.gtiuse, debug= args.debug, show = args.show, nonstop = args.nonstop, lcthresh = args.lcthresh, \
                                            rate_max_ingratio = args.rate_max_ingratio, yscale_ingratio = args.yscale_ingratio, output_dir=args.output_dir)
        ...
        if args.plot_lightcurve:
            plot_lightcurve(event_list, plotpixels, itypenames, args.timebinsize, args.output, ref_time, output_dir=args.output_dir)

        if args.plot_rate_vs_grade:
            plot_rate_vs_grade(event_list, plotpixels, itypenames, args.timebinsize, args.output, ref_time, \
                debug= args.debug, show = args.show, rate_max_ingratio = args.rate_max_ingratio, yscale_ingratio = args.yscale_ingratio, output_dir=args.output_dir)
