...
#                scaled_pulse_to_sub = lo * MEM_AVGS_TO_SUB_ADDR[pixel]/(MEM_AVGS_TO_SUB_ADDR_MAX[pixel]["adc_sample_max"])
                ...
                fig.text(0.1,0.94,f"time={time} prev_interval={pt} next_interval={nt} deriv_max={dmax} pha={pha} pi={pi} risetime={rs} tickshift={ts}")
                fig.text(0.1,0.92,f"quick_double={qd} slope_differ={sd} status={status} slow_pulse={slow_pulse}") 
                if lo > CLIPTHRES:
                    fig.text(0.8,0.92,f"lo_res_ph={lo} > {CLIPTHRES}", color="red")
                else:
                    fig.text(0.8,0.92,f"lo_res_ph={lo}")
