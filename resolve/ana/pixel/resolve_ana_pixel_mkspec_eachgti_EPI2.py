...
#    ene_rebin = rebin  # dE = 0.5 * dPI
    ...
    for itype_ in itypenames:
        ...
        for i, (t1, t2) in enumerate(zip(gtistart, gtistop)):
            ...
            cutid = np.where((itype == itype_) & ((time > t1) & (time <= t2)) & ((epi2 > emin) & (epi2 <= emax)) & np.isin(pixel, plotpixels))[0]
            epi2_filtered = epi2[cutid]

            hist, binedges = np.histogram(epi2_filtered, bins=binnum, range=(emin, emax))
            cumulative_hist += hist
            cumulative_event_number += len(epi2_filtered)
            mgti_dt += dt 
...
#                ene = bincenters * 0.5 + 0.5
                ...
                handle = plt.errorbar(ene, cumulative_hist/(rebin * mgti_dt) + offset, \
                                      yerr=np.sqrt(cumulative_hist)/(rebin * mgti_dt), color=color, fmt='-', 
...
#            ene = bincenters * 0.5 + 0.5
            ene = bincenters
            handle = plt.errorbar(ene, cumulative_hist/(rebin * mgti_dt) + offset, yerr=np.sqrt(cumulative_hist)/(rebin * mgti_dt), color=color, fmt='-', 
...
def main():
    ...
    time, itype, pi, epi2, rise_time, deriv_max, pixel = processed_data  # Unpack data
    ...
    plot_pi(epi2, itype, pixel, time, gtistart, gtistop, emin=args.emin, emax=args.emax, ymin=args.ymin, ymax=args.ymax, rebin=args.rebin, 
            outfname=outfname, 
