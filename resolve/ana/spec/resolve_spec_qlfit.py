...
def parse_args():
    ...
    parser.add_argument('pha', help='pha file that includes rmf, arf file paths.')
    # カンマ区切りの数値列を受け取る
...
################### setting for arguments ###################################################################
    ...
    fname = os.path.basename(pha).replace(".pha", "").replace(".pi", "") + "_" + enetag
    ...
    if procdic["pleff"]:
        # xspec commands
        print(color_text(f"Stage 0: Running xspec to check effective area of {pha}", "blue"))
...
EOF
    ...
    if procdic["plld"]:
        # xspec commands
        print(color_text(f"Stage 1: Running xspec to plot raw spectrum of {pha}", "blue"))
        xspec_script = f"""#!/bin/sh  
...
EOF
    ...
    if procdic["fitpl"]:
        # xspec commands
        print(color_text(f"Stage 2: Running xspec to plot a powerlaw fit of {pha}", "blue"))
        xspec_script = f"""#!/bin/sh  
...
COL 8 ON 2
COL 8 ON 6
LAB 1 COL 8 LIN 0 100 JUS Lef
LAB 2 COL 8 LIN 0 100 JUS Lef
