...
def main():
    # argparse の設定
    parser = argparse.ArgumentParser(description="Extract .gz files based on a CSV file.")
    parser.add_argument("--filename", "-f", type=str, default="data.csv", help="CSV file containing the extraction information.")
    ...
    with open(filename, mode='r', encoding='utf-8') as file:
        ...
        for row in reader:
            if len(row) == 4:
                ...
                # gunzip コマンドを構築
                bash_command = 'find . -type f -name "*.gz" -exec gunzip {} \;'
