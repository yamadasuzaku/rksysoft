...
./resolve_util_fploteve.py $EVENT_FILE DERIV_MAX 1,1,1,1,1,1,1 LO_RES_PH,PHA,EPI,RISE_TIME,ITYPE,PIXEL 1,1,1,1,1,1,1 --filters "PIXEL==${pixel},ITYPE==${itype}" -o ${pixel_str} -c --y_ranges '{"PHA": (0, 65535)}'
...
for itype in 0 1 2; do
    ...
    echo "Processing Pixel $pixel..."
./resolve_util_fploteve.py $EVENT_FILE DERIV_MAX 1,1,1,1,1,1,1 LO_RES_PH,PHA,EPI,RISE_TIME,ITYPE,PIXEL 1,1,1,1,1,1,1 --filters "PIXEL==${pixel},ITYPE==${itype}" -o ${pixel_str} -c --y_ranges '{"PHA": (0, 65535),"EPI":(0,50000)}'
    echo "Completed Pixel $pixel."
done
done 
